...
def post_detail(request : HttpRequest,post_id):
    ...
    if request.method == "POST":
        ...
        if commentform.is_valid():
            added_comment = Comment(post=post, first_name=commentform.cleaned_data["first_name"], content = commentform.cleaned_data["content"], email = commentform.cleaned_data["email"])
            added_comment.save()
        else:
